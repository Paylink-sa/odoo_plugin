...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        payment_link_data = self.provider_id._paylink_make_request('addInvoice', auth=id_token, payload=payload)

        # Extract the payment link URL and embed it in the redirect form.
        rendering_values = {
            'api_url': payment_link_data['url'],
        }
        self.write({'paylink_id_token': id_token, 'paylink_transaction_no': payment_link_data['transactionNo']})
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if not reference:
            raise ValidationError("Paylink: " + _("Received data with missing reference."))

        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'paylinksa')])
        if not tx:
            raise ValidationError(
                "Paylinksa: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_notification_data(self, notification_data):
        ...
        if payment_status == 'pending':
            ...
            self._set_error("Paylink: " + _("Unknown payment status: %s", payment_status))
